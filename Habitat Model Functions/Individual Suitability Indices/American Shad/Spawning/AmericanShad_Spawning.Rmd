---
title: "American Shad Spawning Suitability Indices"
author: "Vanessa Mahan"
date: "2023-04-01"
output: html_document
---
## Depth
American shad are found to spawn over all depths of the river but are found most often from 0.45-12.2 meters (Stier & Crance, 1985). 
```{r}
...
depth_suitability_curve <- function(depth) {
  ...
  optimal_depth_range <- c(0.45, 12.2) # meters
...
American Shad spawn only in freshwater, but have ahigher tolerance and ability to acclimate to higher concentrations of salinity (Stier & Crance, 1985)
```{r}
#optimal Salinity range from 0.0psu-0.5psu
Salinity <- (0.5+5)/2 #average between min & max
...
```
## Temperature
Temperture for spawning around 8-26 degrees celcius with optimal levels of 14-21 degrees celcius (Stier & Crance, 1985). 
```{r}
...
```
## Flow Velocity
Flow velocity was documented as 9.1-132 cm/s with optimal levels around 30.5-91.4 cm/s (Stier & Crance, 1985). Upper and lower limit invalidated to match (Ross et al., 1993). 
```{r}
#depth_range <- c(0.305,0.61)#in meters
flow_velocity <- (0.3-0.5)/2 #average between min & max
flow_velocity_suitability <- function(flow_velocity) {
  # Define the optimal range for depth
  optimal_velocity_range <- c(0, 0.7) # meters/second

  # Define the lower and upper thresholds for depth
  lower_threshold <- 0 # meters/second
  upper_threshold <- 1.32 # meters/second
...
siltclay_suitability <- function(SiltClay_per) {
  # Define the optimal range for depth
  optimal_siltclay_range <- c(0, 0.3) # percent
